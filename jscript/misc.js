...
function addresspicker_init() {
	...
	$('.addresspicker').each(function() {
		...
		var addresspicker = $(this).eb_addresspicker({
			regionBias: "fr",
			elements: {
				map:      '#' + name + '_map',
				lat:      '#' + name + '_lat',
				lng:      '#' + name + '_lng',
				street_number: '#' + name + '_street_number',
				route: '#' + name + '_route',
				locality: '#' + name + '_locality',
				administrative_area_level_2: '#' + name + '_administrative_area_level_2',
				administrative_area_level_1: '#' + name + '_administrative_area_level_1',
				country:  '#' + name + '_country',
				postal_code: '#' + name + '_postal_code',
				type:    '#' + name + '_type'
			}
		});

		var gmarker = addresspicker.eb_addresspicker( "marker");
		gmarker.setVisible(true);
		addresspicker.eb_addresspicker( "updatePosition");

		$(this).eb_addresspicker({
			invalid: function() {
				$( ".addresspicker" ).addClass('invalid_address');
			},

			valid: function() {
				$( ".addresspicker" ).removeClass('invalid_address');
			}
		});
		var address = $(this).val();
		addresspicker.eb_addresspicker('syncAddress', address);

		$(this).focusin(function() {
			$('#' + name + '_eb_map_block').css('top', top + 'px');
		});
		$(this).focusout(function() {
			$('#' + name + '_eb_map_block').css('top', '-2000px');
		});
	});
}